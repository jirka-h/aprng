...
/*
gcc -g -Wall -Wextra -I./ -o Fibonacci_LCG_mix Fibonacci_LCG_mix.c aprng.c tree.c CombGenAPRNG.c -lmylib -ltestu01
valgrind --tool=memcheck -v ./Fibonacci_LCG_mix
gcc -g -Wall -Wextra -I./ -o Fibonacci_LCG_mix Fibonacci_LCG_mix.c aprng.c tree.c CombGenAPRNG.c   -lmylib -ltestu01 -lefence

gcc -O3 -DNDEBUG -Wall -Wextra -I./ -o Fibonacci_LCG_mix Fibonacci_LCG_mix.c aprng.c tree.c CombGenAPRNG.c -lmylib -ltestu01 -lrt -lgsl -lgslcblas
./Fibonacci_LCG_mix 0 0 >(pv >/dev/null )

valgrind --tool=cachegrind ./Fibonacci_LCG_mix
*/
...
inline uint32_t lcg_64_generate_32 (ulcg_CreatePow2LCGL_param_t* d) {
  if ( d->m < 65 ) {
    ...
    unsigned __int128 mult;
    if ( d->c == 0 ) {
      mult = ( (unsigned __int128) d->a * (unsigned __int128) d->s ) % d->m;
    } else {
      mult = ( (unsigned __int128) d->a * (unsigned __int128) d->s + d->c ) % d->m;
    }
    //d->s = ( uint64_t ) ( mult & 0xFFFFFFFFFFFFFFFFULL);
...
int main(int argc, char **argv)
{
  ...
  char name[1024];
  ulcg_CreatePow2LCGL_param_t lcg[2*size];
  ...
  i = 0;
  c = gsl_combination_calloc (SIZEOF(lcg_param), 2);
  do {
    ...
    do {
      ...
      lcg[2*i] = lcg_param[rule[0]];
      lcg[2*i+1] = lcg_param[rule[1]];
      snprintf(name, sizeof(name), "Fibonacci with %s, %s { %d, %d }", lcg_names[rule[0]], lcg_names[rule[1]], rule[0], rule[1]);
      fprintf(stderr, "%s\n", name);

      A[i] = create_Fibonacci();
      L[i].A = A[i];
      L[i].lcg = &lcg[2*i];
      L[i].size = 2;
