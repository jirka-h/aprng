...
/*
gcc -O2 -Wall -Wextra -I./ -o tree tree.c
...
inline void StackInit(stackT *stackP, int maxSize)
{
  ...
  newContents = (stackElementT *)util_Malloc(sizeof(stackElementT) * maxSize);
...
sturm_word_t* sturm_word_new(letter first, letter** const map, const size_t* const length, size_t map_size, size_t max_length) {
  ...
  length_after_expand[0] = (size_t *) util_Calloc (map_size, sizeof(size_t*) ); //Length in the last step
  length_after_expand[1] = (size_t *) util_Calloc (map_size, sizeof(size_t*) ); //Length in the current step
  //length_after_expand[2] is used for the swap


  //fprintf(stderr, "sturm_word_new max_length = %zu\n", max_length);
  sturm_word_t* sturm_word= (sturm_word_t*) util_Malloc ( sizeof(sturm_word_t) );
  ...
  if ( max_length > max[0] ) {
    ...
    size_t **M[4];
    M[0] = (size_t **) util_Calloc (map_size, sizeof(size_t*) ); //Matrix itself
    M[1] = (size_t **) util_Calloc (map_size, sizeof(size_t*) ); //M^(n-1)
    M[2] = (size_t **) util_Calloc (map_size, sizeof(size_t*) ); //M^(n)
                                                            //M[3] used for the swap => no memory allocated
    for (s=0; s<map_size; ++s) {
      M[0][s] = util_Calloc (map_size, sizeof(size_t) );
      M[1][s] = util_Calloc (map_size, sizeof(size_t) );
      M[2][s] = util_Calloc (map_size, sizeof(size_t) );
    }
  ...
  sturm_word->map = (letter**) util_Malloc(map_size * sizeof(letter*) );
  sturm_word->length = (size_t*) util_Malloc(map_size * sizeof(size_t));
  ...
  if ( level > 0 ) {
    ...
    letter* temp[3];
    size_t cur_length[2];
    temp[0] = util_Calloc(max[0], sizeof(letter));
    temp[1] = util_Calloc(max[0], sizeof(letter));
    //temp[3] will be used to swap the pointers

    for(s=0; s<map_size; ++s) {
      sturm_word->map[s] = (letter*) util_Calloc( length_after_expand[0][s], sizeof(letter));
    ...
    for(s=0; s<map_size; ++s) {
      sturm_word->map[s] = (letter*) util_Malloc( length[s] * sizeof(letter));
  ...
  util_Free(length_after_expand[0]);
  util_Free(length_after_expand[1]);
...
void sturm_word_delete(sturm_word_t* data) {
  ...
  for(s=0; s < data->map_size; ++s) {
    util_Free(data->map[s]);
  }
  util_Free(data->map);
  
  util_Free(data->length);
  util_Free(data);
...
uint64_t traverse_with_fill(uint64_t elements, letter* buf, sturm_word_t* data, letter fill, uint64_t where) {
  //Inserts at every where position fill
  //if (data->n % where == 1 ) => fill
  ...
  while(generated<elements) {
    ...
    while ( data->i < data->length[data->a] && generated<elements ) {
      ...
      ++data->i;
      ++data->n;
      if ( generated<elements && data->n%where == 1 ) {
        buf[generated++] = fill;
        ++data->n;
      }
    }
